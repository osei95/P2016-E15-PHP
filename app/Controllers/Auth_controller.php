	...
	class Auth_controller{
		...
		function auth($f3){
			if($f3->exists('POST.username') && $f3->exists('POST.password')){
				...
				$user = $user_model->getUserByUsername($f3, array('username' => $f3->get('POST.username')));
				if($user!=null && $user->user_password == hash('md5', $f3->get('POST.password'), true)){
					...
					$input = $inputs[0];
					$f3->set('SESSION.user', array('user_id' => $user->user_id, 'user_email' => $user->user_email, 'user_firstname' => $user->user_firstname, 'user_lastname' => $user->user_lastname, 'user_key' => $user->user_key, 'user_gender' => $user->user_gender, 'user_description' => $user->user_description, 'access_token' => $input['user_has_input_oauth'], 'access_secret_token' => (($input['user_has_input_oauth_secret']!='NULL')?$input['user_has_input_oauth_secret']:'')));
				}
			}
		}
		...
		function register($f3){
			...
			if(count($errors)==0){
				...
				$input_model = new Input_model();
				$input_model->newInput($f3, array('user_id' => $user->user_id, 'input_key' => $registration_infos['input_id'], 'input_name' => $registration_infos['input_name'], 'oauth' => $registration_infos['access_token'], 'oauth_secret' => (isset($registration_infos['access_secret_token'])?$registration_infos['access_secret_token']:'')));
				...
				$input_model = new Input_model();
				$input_model->updateOauth($f3, array('user_has_input_id' => $registration_infos['input_id'], 'oauth' => (isset($registration_infos['access_token'])?$registration_infos['access_token']:'NULL')));
				$f3->reroute('/');
			}else{
